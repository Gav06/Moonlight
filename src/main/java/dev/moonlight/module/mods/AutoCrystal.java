...
public class AutoCrystal extends Module {
    ...
    public void onUpdate(PlayerUpdateEvent event) {
        ...
        System.out.println("NIGGER BEATER 420");
        ...
        if(targetPlayer != null) MessageUtil.sendMessage("GOT TARGET");
        ...
        if (placeTimer.passedMs((long) placeDelay.getValue())) {
            doPlace();
            MessageUtil.sendMessage("ATTEMPTING PLACE");
            placeTimer.reset();
        }
        if (breakTimer.passedMs((long) breakDelay.getValue())) {
            doBreak();
            MessageUtil.sendMessage("ATTEMPTING BREAK");
            breakTimer.reset();
        }
    }
